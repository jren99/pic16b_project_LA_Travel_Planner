...
* Jaya Ren: I built the first version of the webapp using php and html (see in folder `django and php test files`) and worked with Stancy to build the final version of the webapp using flask, which enables different filters for hotels, attractions, and restaurants, the generated route as well as its summary, and a submission form for users to contact us.
